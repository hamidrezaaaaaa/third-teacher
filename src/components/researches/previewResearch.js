      ...
      response.blob().then((blob) => {
        if (state.loggedIn) {
          var data = JSON.stringify({
            ...
            productId: `${research.researchId}`,
            orderDate: `${new Date().toLocaleString("fa")}`,
            productName: `${research.title}`,
            tag: `research`,
          });
          ...
          axios(config)
            .then(function (response) {
              ...
              alink.href = fileURL;
              alink.download = `${BaseBackURL}documents/${research.pdf}`;
              alink.click();
            })
            .catch(function (error) {
              console.log(error);
  ...
  return (
    <Container>
      ...
      <Content>
        ...
        <p className="text">{research && research.summary}</p>

        <Download onClick={onButtonClick}>دریافت پژوهش</Download>
      </Content>
      <Cover>
        <img src={`${BaseBackURL}uploads/${research.image}`} alt={research.title} />
      </Cover>
...
export default PreviewResearch;
