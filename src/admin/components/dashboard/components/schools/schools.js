...
const Schools = () => {
  ...
  const tableCell = schools.map((item, i) => {
    return (
      <tr key={i}>
        <td>{item.id}</td>
        <td>{item.name}</td>
  ...
  return (
    <Container>
      ...
      {/* addPhilosopher modal */}
       <AddSchool onClose={setAddModal} visible={addModal} />
      

       {/* editPhilosophe modal */}
       <EditSchool onClose={setEditModal} visible={editModal} school={selectSchool} />
      ...
      >
        <DeleteContainer>
          <h3>آیا از حذف مدرسه ذیل اطمینان دارید؟</h3>
          <div className="row">
            <p className="text">نام مدرسه</p>
            <p className="text">{selectSchool.name}</p>
          </div>
          <div className="submit-row">
            <button
              className="confirm"
              onClick={() => {
                deleteSchool(selectSchool.id);
                setDeleteModal(false);
              }}
            >
              تایید
...
const AddSchoolButton = styled.div`
